...
class WaveletDiffusionTransformer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Check input for NaN
        # Input NaN check removed for torch.compile reduce-overhead compatibility
        # Was: if torch.isnan(x_0).any(): ...
        
        t = torch.randint(0, self.T, (x_0.size(0),), device=self.device)
        loss = self.compute_loss(x_0, t)
        
        # Enhanced loss monitoring and stability
        # Loss NaN check removed for torch.compile reduce-overhead compatibility
        # Was: if torch.isnan(loss) or torch.isinf(loss): ...
